...
import { compare, renderDigest } from "./digest.js";
import { tryCandidate } from "./isolate.js";
import { validateRegistry } from "./registry.js";
import { apply, findOpen, plan, render, verdictOf } from "./report.js";
import { ageHours, resolve } from "./resolve.js";
import { renderSurvey, survey } from "./survey.js";
import { loadWatches, runWatch, watchRow } from "./watch.js";
...
function loadRegistry() {
  ...
  if (!existsSync(path))
    throw new Error(`no registry at ${path}; pass --registry or create timbrado.json`);
...
async function main() {
  switch (cmd) {
    case "observe": {
      if (!positional[0])
        throw new Error("usage: timbrado observe <opportunities.json> [--json out]");
      const args = ["observe", resolvePath(positional[0])];
      if (flag("--json"))
        args.push("--json", resolvePath(flag("--json")));
    ...
    case "survey": {
      ...
      if (flag("--json"))
        writeFileSync(flag("--json"), JSON.stringify(rows, null, 2) + `
`);
    ...
    case "resolve": {
      ...
      for (const name of names) {
        ...
        if (!t) {
          console.error(`no target ${name} in the registry`);
          code = 2;
          continue;
        }
        try {
...
${"".padEnd(16)} binary:  ${c.binary.url}` : ""}`);
    ...
    case "fetch": {
      ...
      if (!name || !into)
        throw new Error("usage: timbrado fetch <target> --into <dir>");
      const c = await resolve(name, reg.targets[name]);
      if (!c.binary)
        throw new Error(`${name} resolves to no binary (kind ${c.kind})`);
      ...
      if (c.binary.integrity) {
        ...
        if (got !== c.binary.integrity)
          throw new Error(`integrity mismatch: registry says ${c.binary.integrity}, downloaded ${got}`);
      }
    ...
    case "try": {
      ...
      if (!what || !gate)
        throw new Error('usage: timbrado try <target|spec> --gate "<cmd>"');
      const root = resolvePath(flag("--repo") ?? ".");
      let spec = what;
      let subject = { spec: what };
      if (existsSync(flag("--registry") ?? "timbrado.json")) {
        const reg = loadRegistry();
        if (reg.targets[what]) {
          const c = await resolve(what, reg.targets[what]);
          if (!c.install)
            throw new Error(`${what} resolves to nothing installable (kind ${c.kind})`);
          spec = c.install;
      ...
      const report = {
        schemaVersion: 1,
        target: String(subject.target ?? what),
        verdict: r.verdict,
...
`);
    ...
    case "digest": {
      ...
      const to = flag("--to");
      if (!repo || !from || !to)
        throw new Error("usage: timbrado digest --repo <owner/name> --from <sha> --to <sha>");
      let text;
      try {
        ...
        if (process.env.GITHUB_ACTIONS)
          console.error(`::warning title=digest unavailable::${why}`);
...
`;
    ...
    case "watch": {
      ...
      if (!pinned || !candidate)
        throw new Error("usage: timbrado watch --pinned <exe> --candidate <exe> [--watches file]");
      const list = await loadWatches(pathToFileURL(resolvePath(flag("--watches") ?? "timbrado.watches.ts")).href);
      const rows = [];
      const mark = (v) => v === null ? "?" : v ? "landed" : "not yet";
      for (const w of list) {
        ...
        if (row.pinned === null || row.candidate === null || note.includes("moved"))
          console.log(`       ${row.detail}`);
      }
      const v = verdictOf([], rows);
      const report = { target: flag("--target") ?? "watches", ...v, subject: { pinned, candidate }, watches: rows };
      if (flag("--json"))
        writeFileSync(flag("--json"), JSON.stringify(report, null, 2) + `
`);
    ...
    case "report": {
      ...
      if (!target || !jsonPath)
        throw new Error("usage: timbrado report --target <name> --json <report> [--exit <n>]");
      const exit = Number(flag("--exit") ?? "0");
      let report;
      try {
        report = JSON.parse(readFileSync(jsonPath, "utf8"));
      } catch {
        console.error(`::error title=${target} wrote no report::exit ${exit}`);
        return 2;
      }
      if (report.verdict === "instrument" || exit !== 0 && exit !== 1) {
        console.error(`::error title=${target} could not run::${report.reason ?? `exit ${exit}`}`);
        return 2;
      }
      const repo = flag("--repo") ?? undefined;
      const open = findOpen(target, repo);
      const action = plan({ ...report, target }, open);
